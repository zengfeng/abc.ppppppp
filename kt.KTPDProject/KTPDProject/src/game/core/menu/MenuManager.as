package game.core.menu
{
	...
	public class MenuManager
	{
		...
		public function getMenu(index : int = 1) : GComponent
		{
			switch(index)
			{
				case 0:
		 ...
		 * 功能是否开启
		 */
		public function checkOpen(index : int) : Boolean
		{
			var tempButton : MenuButton = menuButton[index];
			if (!tempButton) return false;
			return checkOpenByVo(tempButton, false);
		}

		// private
		internal function checkButton(button : MenuButton) : Boolean
		{
			if (button.vo.level < 1000 && UserData.instance.level >= button.vo.level) return true;
			if (button.vo.level > 1000 && QuestManager.getInstance().isSubmit(button.vo.level)) return true;
			return false;
		}
		...
		private function refreshShowingView() : void
		{
			...
			w = (UIManager.stage.stageWidth - w) / 2;
			for each (button in _showIngView)
			{
				if (button.vo.stack != MAIN_STACK || w == button.target.x) continue;
				TweenLite.to(button.target, 0.5, {x:w, overwrite:0});
				w += (button.target is GCommonWindow) ? button.target["outWidth"] : button.target.width;
			}
		}
		...
		private function addButton(vo : VoMenuButton, type : int = 0) : MenuButton
		{
			...
			if (type == 0)
			{
				_data.downAsset = new AssetData(vo.ioc + "Click", "embedFont");
				_data.overAsset = new AssetData(vo.ioc + "Over", "embedFont");
				_data.upAsset = new AssetData(vo.ioc, "embedFont");
				_data.parent = _bottomMenu;
			}
			var button : MenuButton = new MenuButton(_data, vo, type);
