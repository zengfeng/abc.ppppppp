package game.core.avatar {
	...
	public class AvatarFight extends AvatarThumb
	{
		...
		// 设置动作
		override public function setAction(value : int, loop : int = 1, index : int = 0, arr : Array = null) : void
		{
		...
		public function getIsDie():Boolean { return _isDie; }
		
		protected var _skillAvatar : AvatarSkill;   //avatar身上的技能
		 
		protected var _skillGroundBuf:AvatarSkill;  //avatar身上的地面技能buff
		...
		override public function die() : void
		{
			...
			for each(var buff:AvatarSkill in _buffDic)
			{
				...
				///Logger.debug(this._id+"buff.stop()");
			}
			_isDie=true;
		...
		public function playSkill(spellEftid : int, playType:uint, frontOrback:uint, needTurnOver:Boolean, angle:uint,side:int) : void
		{
...
//						}
			...
			///Logger.debug("PLAYSKILL==>"+playType,"typeID===>"+spellEftid, "uuid===>"+AvatarManager.instance.getUUId(spellEftid, firstKeyID, frontOrback));
		...
		//
		public function playupdownwords(skillid:int, control:uint):void
		{
		...
		//index 0:底层 1：顶层
		public function playBuff(skillid : int, control : uint) : void
		{
			if(skillid == skillType.GAUGEFULL) //聚气特殊处理，有两个buff效果组合
			{
				//Logger.debug("skillId===>"+skillid,"avatarId===>"+_id,"control===>"+control);
				...
				switch(control)
				{
					case 0: //移除
					{
						...
						skillGaugeB.dispose();
						skillGaugeB.hide();
						//Logger.debug(this._id+"buff.hide2()");
						break;
					}
					case 1: //添加
					{
		...
		public function playgroudeft(skillid:uint, myside:uint):void
		{
			...
			if(groundeftid > 0)
			{
				gEftAvatar = new AvatarSkill();
				gEftAvatar.initAvatar(groundeftid, AvatarType.SKILL_TYPE_GROUND, frontOrback);
				gEftAvatar.player.addEventListener(Event.COMPLETE, skillgroundSkillComplete); //控制播放一次
				gEftAvatar.setAction(1, 1, 0);
				addChild(gEftAvatar);
				gEftAvatar.show();
		...
		private function createBuffAvatar(id : int, putposition:int = 0) : AvatarSkill
		{
			...
			switch(skillType.getTypeOfSkillType(id))
			{
				case 0:
					skillAvatar.x = 0;
					skillAvatar.y = 0;
		...
		private function skillComplete(event : Event) : void
		{
			if(_skillAvatar)
			{
				///Logger.debug("skillComplete    UUID====>" + _skillAvatar.id);
				_skillAvatar.player.removeEventListener(Event.COMPLETE, skillComplete);
				_skillAvatar.hide();
			}
		}
		
		private function skillgroundSkillComplete(event:Event):void
		{
			if(_skillGroundBuf)
			{
				_skillGroundBuf.player.removeEventListener(Event.COMPLETE,skillgroundSkillComplete);
				_skillGroundBuf.hide();
			}
		}

		private function onComplete(event : Event) : void
		{
			//trace("onComplete AvatarFight setAction  id===>"+id,"value===>"+_action);
			if (_action == AvatarManager.DIE)
			{
				this.player.stop();
			}
			else
...
//		override protected function setUUID(id : int) : void
//		{
//			super.setUUID(id);
//			addProgressBar();
//		}
