package game.core.item.soul
{
	...
	public class Soul extends EquipableItem
	{
		...
		public function get flameId() : uint
		{
		...
		private function popupMergeAlert() : void
		{
			...
			else if (strong.exp + weak.exp >= strong.upgradeExp)
			{
				...
				StateManager.instance.checkMsg(116, [strong.htmlName, weak.htmlName, exp, upgradeLevel], popupMergeAlertCallback);
				SoulDragManager.state = SoulDragManager.DRAGGING;
			}
			else
			{
				SoulDragManager.state = SoulDragManager.DRAGGING;
				StateManager.instance.checkMsg(115, [strong.htmlName, weak.htmlName, weak.exp], popupMergeAlertCallback);
			}
		}
