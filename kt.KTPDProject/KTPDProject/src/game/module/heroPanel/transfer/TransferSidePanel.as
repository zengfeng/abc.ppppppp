package game.module.heroPanel.transfer {
	...
	public class TransferSidePanel extends GCommonSmallWindow
	{
		...
		{
			_sourceItemName = UICreateUtils.createTextField("原有装备", null, 120, 0, 43, 16, TextFormatUtils.panelSubTitle);
		...
		{
			_targetItemName = UICreateUtils.createTextField("目标装备", null, 120, 0, 191, 16, TextFormatUtils.panelSubTitle);
			...
			_targetLevelText = UICreateUtils.createTextField(null, null, 120, 0, 184, 118, TextFormatUtils.panelContent);
		...
		private function addExpense() : void
		{
			...
			contentPanel.addChild(UICreateUtils.createTextField("所需材料：", null, 0, 0, 18, 141, TextFormatUtils.panelContent));
			...
			if (_model.mode == TransferModel.MODE_TRANSFER)
			{
				...
				else
				{
					...
					if (item && item.nums < _model.costNums)
						SingleQuickShop.show(item.id, ID.GOLD, 1, processTransfer, null, this);
					else
					{
						processTransfer();
					}
