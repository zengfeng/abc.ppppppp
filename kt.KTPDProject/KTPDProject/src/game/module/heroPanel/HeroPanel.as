package game.module.heroPanel
{
	 ...
	 * @author yangyiqiang
	 */
	public class HeroPanel extends GCommonWindow
	{
		// ===============================================================
		// 定义
		// ===============================================================
		...
		{
			var bg : Sprite = UIManager.getUI(new AssetData("soul_power_bg"));
			bg.width = 140;
			bg.x = 96;
			bg.y = 15;
		...
		{
			_heroSoul = UIManager.getUI(new AssetData("soul_power_bg"));
			_heroSoul.width = 140;
			_heroSoul.x = 256;
			_heroSoul.y = 15;
		...
		protected function dissmissButton_ClickHandler(event : MouseEvent) : void
		{
			if (_selectedHero.equipments.length > 0 || _selectedHero.souls.length > 0)
				StateManager.instance.checkMsg(96, [_selectedHero.htmlName], onConfirmDissmiss);
			else
				StateManager.instance.checkMsg(95, [_selectedHero.htmlName], onConfirmDissmiss);
		}
