package game.module.quest
{
	...
	public final class QuestManager
	{
		...
		{
			loopCount = (DailyInfoManager.instance.dailyValue & 0x7f000000)>>24;
			...
			for (var i : int = 0;i < max;i++)
			{
				...
				{
//					if(temp.base.type==2){
//						QuestUtil.sendTaskOperateReq(temp.id, 1);
//						continue;
//					}
...
//			if (vo.state == CAN_ACCEPT&&vo.getStep()==0&&vo.stepMessages.length>0)
		...
		public function submitQuest(taskId : int) : Array
		{
			...
			for each (var id:int in tempArray)
			{
				...
				{
//					if(vo.base.type==2){
//						QuestUtil.sendTaskOperateReq(vo.id, 1);
//						continue;
//					}
...
//					}
		...
		public function findCurrentDailyQuest() : VoQuest
		{
			var max : int = _dailyList.length;
			for (var i : int = max - 1;i > -1;i--)
			{
				if (_dailyList[i].base.playerLevel <= UserData.instance.level && _dailyList[i].id != COMMON_DAILY)
					return _dailyList[i];
			}
			return _dailyList[0];
		}

		public function toDaily() : Boolean
		{
			for each (var vo:VoQuest in _dailyList)
			{
				if (vo.state == CAN_ACCEPT || vo.state == CURRENT)
				{
					QuestUtil.questVoAction(vo);
					if (vo.id == COMMON_DAILY) return true;
					else return false;
				}
			}
			return false;
		...
		private function isCanAccept(vo : VoQuest) : Boolean
		{
			...
			{
				if (QuestManager.getInstance().loopCount <= 0) return false;
