package game.module.quest
{
	...
	public final class QuestUtil {
		...
		public static function questVoAction(vo : VoQuest) : void {
			...
			if (vo.isCompleted) {
				...
				if (vo.base.getCompleteTry() == 7 || vo.base.getCompleteTry() == 8) {
					FollowManager.instance.removeNpcFollowSelf(vo.base.npcPublish);
					QuestUtil.removeQuestMode();
				}
				return;
			}
			if (vo.base.npcPublish != 0 && vo.getStep() == 0 && vo.base.type == 3) {
				goNpc(vo.base.npcPublish);
				return;
			}
			...
			switch(completeType) {
				...
				case 9:
				case 10:
					goNpc(vo.base.step.split(",").shift());
					break;
				case 4:
				...
				case 5:
					...
					_type = 0;
					startAutoRun(vo.id);
					MapSystem.mapTo.toMap(str[1], str[2], str[3], false, startUserItem);
					break;
				case 6:
					str = vo.base.step.split(",");
					structId = str[0];
				...
				case 8:
					...
					if (!isIn) {
						...
						if (!tempBase) return;
						MapSystem.selfPlayerAnimal.avatar.setClanName("护送" + tempBase.name + "中...", QUEST_COLOR);
						FollowManager.instance.npcFollowSelf(completeType == 7 ? vo.base.npcPublish : str[0]);
						isIn = true;
					}
					break;
				case 11:
				...
				case 12:
					str = vo.base.step.split("|");
					startAutoRun();
					MapSystem.mapTo.toDuplNpc(str[0]);
					//MapSystem.mapTo.toDupMap(str[0], false, endAutoRun, null, null, 2, null);
					break;
				case 13:
					// 去副本打怪,打怪前出现剧情
					str = vo.base.step.split(",");
					startAutoRun();
					MapSystem.mapTo.toDuplNpc(str[0]);
					//MapSystem.mapTo.toDupMap(str[0], false, endAutoRun, ActionDriven.instance().playAnimation, [vo.base.plotId], 10, [str[1], str[2]]);
				default :
					goNpc(vo.base.step.split(",").shift());
					break;
			}
		...
		private static function playProgress(str : String) : void {
			...
			_avatar = AnimalManager.instance.selfPlayer.avatar;
		 ...
		 */
		private static function goNpc(npcId : int, questId : int = 0, type : int = 0, isRun : int = 0, flashStep : Boolean = false) : void {
			var tempBase :VoNpc = RSSManager.getInstance().getNpcById(npcId);
			if (!tempBase) return;
			structId = npcId;
			startAutoRun(questId, isRun);
			type == 0 ? MapSystem.mapTo.toNpc(npcId, tempBase.mapId, flashStep, npcClick, [0, questId]) : MapSystem.mapTo.toNpc(npcId, tempBase.mapId, flashStep, sendQuestComplete, [questId]);
		}
		...
		private static function startAutoRun(id : int = 0, isRun : int = 0) : void {
			if (!MapSystem.selfPlayerAnimal || !MapSystem.selfPlayerAnimal.avatar) return;
			_currnQuestId = id;
			if (isRun != 0) return;
			MapSystem.selfPlayerAnimal.avatar.setClanName("自动寻路中...", "#00ee66");
			_lastTime = getTimer();
		...
		private static function onClick(event : MouseEvent) : void {
			...
			if (++_num == 1) return;
			MapController.instance.onClickMap(event);
			endAutoRun();
			checkQuide();
		}
		...
		public static function endAutoRun() : void {
			if (!MapSystem.selfPlayerAnimal || !MapSystem.selfPlayerAnimal.avatar) return;
			MapSystem.selfPlayerAnimal.avatar.setClanName("");
			_lastTime = 0;
			_num = 0;
		...
		public static function checkQuide() : void {
			if (GetValManager.guide_inDuplDefeatBoss()) return;
			...
			if ((!vo || !QuestDisplayManager.getInstance().isShowing(vo)) && !isInAutoRun && !MapUtil.isDungeonMap()) {
		...
		public static function npcClick(id : int = 0, questId : int = 0) : void {
			id = id == 0 ? structId : id;
			if (id == 3002) {
				if ( GuildManager.instance.actionEnable(2) ) {
			...
			cmd.npcId = id;
			if (questId != 0) cmd.conveyId = questId;
