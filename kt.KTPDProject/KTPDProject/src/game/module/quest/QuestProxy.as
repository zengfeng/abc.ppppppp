package game.module.quest {
	...
	public class QuestProxy {
		...
		private function taskTrigger(msg : SCQuestOperateReply) : void {
			...
			switch(msg.op) {
				...
				case 2:
					...
					(AnimalManager.instance.selfPlayer.avatar as AvatarMySelf).questComplete();
					_manage.resetQuestNpcState(temp);
		...
		private function taskAction(msg : SCQuestStepUpdate) : void {
			...
			Logger.debug("SCQuestStepUpdate");
			if (temp.isCompleted) {
		...
		private function npcTrigger(msg : SCNpcTrigger) : void {
			...
			// ActionDriven.instance().playAnimation(100003);
			// ActionDriven.instance().playAnimation(102003);
			var str : String = "";
			...
			if (msg.acts.length == 0) {
				if (npc.type == 1 && MapSystem.getNpcAvatar(npc.id))
					MapSystem.getNpcAvatar(npc.id).showDialog(npc.defaultDialog);
			} else {
				var actionTyp : int;
				if (quest&&quest.base.type == 3) {
		...
		private function updateMsg(msg : SCLoopQuestSubmitRes) : void {
			QuestManager.getInstance().loopCount = msg.leftNum;
			SignalBusManager.updateDaily.dispatch(DailyManage.ID_QUEST, msg.leftNum > 0 ? DailyManage.STATE_OPENED : DailyManage.STATE_ENDED, msg.leftNum);
		}
	}
}
