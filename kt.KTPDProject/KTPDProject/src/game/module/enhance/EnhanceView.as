...
{
	import game.core.avatar.AvatarUnit;
	import bd.BDData;
	import bd.BDUnit;

	import game.core.avatar.AvatarBD;
	 ...
	 * @author verycd
	 */
	public class EnhanceView extends GCommonWindow
	{
		// ===============================================================
		// 属性
		// ===============================================================
		...
		private function addEffectPlayer() : void
		{
			...
			data.x = 164;
			data.y = 102;
			_effectPlayer = new EffectPlayer(data);
			var uuid:int=AvatarManager.instance.getUUId(1, AvatarType.COMMON_EFFECT);
			var bds:BDData = AvatarManager.instance.getAvatarBD(uuid).bds;
			if (bds.total < 2)
				throw(Error("加载错误！"));
			_effectPlayer.setBDData(bds);
		}
		...
		{
			var bitmapData : BitmapData = RESManager.getBitmapData(new AssetData(UI.ENHANCE_CIRCLE_BACKGROUND));
		...
		{
			_noItemText = UICreateUtils.createTextField("请放入装备", null, 150, 0, 100, 133, TextFormatUtils.panelSubTitleCenter);
		...
		{
			var silver : Item = ItemManager.instance.getPileItem(ID.SILVER);
		...
		{
			_subPanel.addChild(UICreateUtils.createTextField("可强化装备", null, 160, 0, 336, 4, TextFormatUtils.panelSubTitle));
		}
		...
		private function updateCostStone() : void
		{
			...
			else
			{
				...
				_enhanceStone.source = ItemManager.instance.newItem(ID.ENHANCE_STONE_0);
				_enhanceStoneText.htmlText = "消耗强化石";
			}
		...
		{
			if (_model.item && !_model.hitLimit)
			{
