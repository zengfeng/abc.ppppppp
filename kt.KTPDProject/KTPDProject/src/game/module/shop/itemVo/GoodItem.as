...
//		private var img:GImage;
		...
		private function onSubmit(event : MouseEvent) : void {
			var mygold:int = UserData.instance.gold;
			if(mygold<(int(enterText.text) * int(goodsPrice.text))){
				StateManager.instance.checkMsg(4);
				return;
			}
			StateManager.instance.checkMsg(287,[(int(enterText.text) * int(goodsPrice.text)),goodsItem.htmlName,int(enterText.text)],alertCallFun);
		}
