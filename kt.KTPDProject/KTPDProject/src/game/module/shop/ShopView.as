package game.module.shop
{
	...
	import game.module.shop.mysteriousShop.MysteriousShop;
...
//		private var honourGoodsPanel:HonourExchangePanel;
		...
		//神秘商店面板
		private var mysterioursSohp:MysteriousShop;
		...
		private function onMysterousPanel(event : MouseEvent) : void {
			if(!mysterioursSohp){
				mysterioursSohp = new MysteriousShop();
				MenuManager.getInstance().getMenuButton(MenuType.MYSTERIOUSSHOP).target=mysterioursSohp;
			}
			MenuManager.getInstance().changMenu(MenuType.MYSTERIOUSSHOP);
		}
		...
		//		mysteriousBtn.visible= true;
			...
			mysteriousBtn.addEventListener(MouseEvent.MOUSE_DOWN, onMysterousPanel);
