package game.module.pack
{
	...
	public class PackView extends GCommonWindow
	{
		...
		private function addTabbedPanel() : void
		{
			...
			(_tabbedPanel.group.model.getLast() as GTab).source = Item.EQ;
		...
		{
			var bar : Sprite = UIManager.getUI(new AssetData(UI.PACK_EXPAND_BAR_BG));
			bar.x = 20;
			bar.y = 379;
			bar.width = 114;
			_itemsPanel.add(bar);

			_expandButtonBg = UIManager.getUI(new AssetData(UI.PACK_EXPAND_BUTTON_BG));
			_expandButtonBg.x = 134;
			_expandButtonBg.y = 375;
			_itemsPanel.add(_expandButtonBg);

			var data : GButtonData = new GButtonData();
			data.scaleMode = ScaleMode.SCALE_NONE;
			data.upAsset = new AssetData(UI.PACK_EXPAND_BUTTON_UP);
			data.downAsset = new AssetData(UI.PACK_EXPAND_BUTTON_DOWN);
			data.overAsset = new AssetData(UI.PACK_EXPAND_BUTTON_OVER);
			data.x = 135;
			data.y = 380;
			data.width = 16;
			data.height = 16;

			_expandButton = new GButton(data);
			_itemsPanel.add(_expandButton);
		...
		protected function initEvents() : void
		{
			...
			_expandButton.addEventListener(MouseEvent.CLICK, expandButton_clickHandler);
			...
			// _mergeAllButton.addEventListener(MouseEvent.CLICK, mergeAllButton_clickHandler);
			...
			ToolTipManager.instance.registerToolTip(_expandButton, ToolTip, getExpandTipString);
		...
		private function removeEvents() : void
		{
			...
			_expandButton.removeEventListener(MouseEvent.CLICK, expandButton_clickHandler);
			...
			_itemGrid.addEventListener(ItemCellEvent.SELECT, itemGrid_selectCellHandler, true);
			// _mergeAllButton.removeEventListener(MouseEvent.CLICK, mergeAllButton_clickHandler);
			...
			ToolTipManager.instance.destroyToolTip(_expandButton);
		...
		{
			StateManager.instance.checkMsg(77, [StringUtils.addColorById(getExpandCost().toString(), ColorUtils.ORANGE)], expandButton_callback);
		}
		...
		{
			return "花费" + StringUtils.addColorById(getExpandCost().toString(), ColorUtils.ORANGE) + "元宝扩展50格包裹容量";
		}
	}
}
