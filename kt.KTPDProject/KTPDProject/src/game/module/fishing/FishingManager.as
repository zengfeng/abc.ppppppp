package game.module.fishing
{
	...
	import game.module.fishing.view.AwardPanel;
	import game.module.fishing.view.BaitPanel;
	import game.module.fishing.view.BasketPanel;
	import game.module.fishing.view.ProgressPanel;
	...
	public class FishingManager
	{
		...
		public static const BOX : String = 'fishBox';
		// ===============================================================
		// 单例
		// ===============================================================
		...
		private var _fisherAvatar : AvatarFisher;
		...
		// Getter/Setter
		// ===============================================================
		public function set fisherAvatar(value : AvatarFisher) : void
		{
			_fisherAvatar = value;
			if (_fisherAvatar && _pendingShowProgress)
			{
				showProgressPanel();
			}
		}
		...
		public function enterFishing(type : uint) : void
		{
			if (type == 1)
				_model.resume = false;
			else
			...
			addEvents();
			// transferPlayer();
			// sendFishReadyMessage();
			EnWalkManager.addCheckEnWalk(onWalkLeaveFishing);
		}
		...
		private function showProgressPanel() : void
		{
			...
			{
				// progressPanel.y = -90;
				progressPanel.x = _fisherAvatar.parent.x;
				progressPanel.y = _fisherAvatar.parent.y - 90;
				progressPanel.setPosition(_fisherAvatar.getPosition());
				if (progressPanel.parent == null) MapController.instance.elementLayer.addForegroundEffect(progressPanel);
		...
		private function showAwardPanel() : void
		{
			...
			awardPanel.x = -190;
			awardPanel.alpha = 0;
			_fisherAvatar.addChild(awardPanel);
		...
		private function hideProgressPanel() : void
		{
		}
		...
		{
			StateManager.instance.checkMsg(22, [10], checkSoonDone);
			function checkSoonDone(type : String) : Boolean
			{
				switch(type)
				{
					...
					case Alert.YES_EVENT :
						if (UserData.instance.trySpendGold(10))
		...
		private function onCancelFish(msg : SCCancelFishRes) : void
		{
			...
			if (_baitPanel)
				_baitPanel.hide();
			removeEvents();
			EnWalkManager.removeCheckEnWalk(onWalkLeaveFishing);
		}
		...
		private function onFishDraw(msg : SCFishDraw) : void
		{
			...
			if (msg.ret == 0)
			{
				...
				_fisherAvatar.pull(award.imgUrl, onPullComplete);
			}
			// 包裹已满
		...
		private function checkWalkLeaveFishing(type : String) : Boolean
		{
			if (type == Alert.YES_EVENT || type == Alert.OK_EVENT)
			{
				canelFishing();
				EnWalkManager.removeCheckEnWalk(onWalkLeaveFishing);
				EnWalkManager.continueWalk.dispatch();
				// SignalBusManager.mapWorldOpen.dispatch();
			}
