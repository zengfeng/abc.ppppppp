package game.module.fishing.view
{
	import game.core.user.StateManager;
	import game.definition.UI;
	import game.module.fishing.FishingManager;
	import game.module.fishing.FishingModel;
	import game.module.fishing.FishingPosition;
	import game.module.fishing.FishingState;
	...
	public class ProgressPanel extends GComponent
	{
		...
		private function addTexts() : void
		{
			...
			_timeText = UICreateUtils.createTextField('钓鱼剩余时间', null, 180, 20, -70, -82, TextFormatUtils.content);
		...
		{
			var basketButtonClass : Class = RESManager.getClass(new AssetData("fishing_basket_button"));
			_basketButton = new basketButtonClass() as SimpleButton;
			_basketButton.x = -90;
			_basketButton.y = -60;
			addChild(_basketButton);

			var startButtonClass : Class = RESManager.getClass(new AssetData("fishing_rod_button"));
			_startButton = new startButtonClass() as SimpleButton;
			_startButton.x = -23;
			_startButton.y = -60;
			addChild(_startButton);

			var dragButtonClass : Class = RESManager.getClass(new AssetData("fishing_bait_button"));
			...
			_pullEffect = RESManager.getMC(new AssetData(UI.FIRE_CIRCLE));
//			_pullEffect = new BDPlayer(new GComponentData());
//			_pullEffect.mouseEnabled = false;
//			_pullEffect.setBDData(BDUtil.getBDData(new AssetData(UI.FIRE_CIRCLE)));
			_pullEffect.mouseEnabled = false;
			_pullEffect.x = -28.5;
			_pullEffect.y = -62.5;
			_pullEffect.scaleX = 0.65;
			_pullEffect.scaleY = 0.65;
			addChild(_pullEffect);
			

			_exitButton = UICreateUtils.createGButton(null, 0, 0, -23, -60, KTButtonData.EXIT_BUTTON);
		...
		{
			_timeText.text = "钓鱼剩余时间 " + TimeUtil.secondsToTime(_model.totalSeconds - _model.timer.currentCount);
		}
