	...
	public class ArtifactMainPanel extends GComponent
	{
		...
		private function initButton() : void
		{
			for (var i : int = 1;i < 5;i++)
			{
				...
				if (i == 3) ToolTipManager.instance.registerToolTip(_buttonDic[i - 1], SmallTip, "花费" + StringUtils.addColor("200", ColorUtils.HIGHLIGHT_DARK) + "元宝铸魂神器，暴击率" + StringUtils.addColor("75%", ColorUtils.HIGHLIGHT_DARK) + "，获得的经验比免费更多");
				if (i == 2) ToolTipManager.instance.registerToolTip(_buttonDic[i - 1], SmallTip, "免费铸魂神器，暴击率" + StringUtils.addColor("50%", ColorUtils.HIGHLIGHT_DARK));
			}
		}
		...
		private function initCorona() : void
		{
			if (!_corona)
			{
				...
				ToolTipManager.instance.registerToolTip(_tipsMode, SmallTip, getExpTips);
			}
		...
		private function changButton() : void
		{
			...
			switch(_selectVo.type)
			{
				...
				case 2:
					...
					addChild(_buttonDic[2]);
					_critText.htmlText = ArtifactManage.instance().getCritNumString();
					addChild(_critText);
					break;
				case 3:
					_countText.htmlText = "";
					addChild(_buttonDic[3]);
					initCorona();
			...
			ToolTipManager.instance.registerToolTip(_img, SmallTip, _selectVo.getTips());
		}
	}
}
