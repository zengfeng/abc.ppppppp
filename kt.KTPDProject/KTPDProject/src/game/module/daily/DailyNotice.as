...
//		private var _button:NoticeButton ;
		...
		private function set shown(value:Boolean):void{
			if( value != _isShown ){
				_isShown = value ;
				if( _isShown ){
					UIManager.root.addChild(_window);
				}
				else {
					UIManager.root.removeChild(_window);
				}
			}
		}
...
*/
		...
		private static function BossFightBegin( msg:SCBossFightBegin ) : void {
			if( MenuManager.getInstance().checkOpen(MenuType.DAILY) && !MapUtil.isBossWarMap() )
			{
				if( msg.flag == 2 || msg.flag == 3 )
					instance.showNotice(1, 1,0 , function():void{actionShortCut(5);} );
				else if( msg.flag >= 4 )
					instance.showNotice(1, 0, msg.flag-4 ,  function():void{actionShortCut(5);} ) ;
			}
		}
		...
		private static function GroupBattleNotice(msg:SCGroupBattleTime) : void {
			...
			{
				if( msg.status == 1 )
					instance.showNotice(6, 1 , 0 , function():void{actionShortCut(6);});
...
//			ViewManager.instance.addToStage(_window);
		...
		{
			var vos:Vector.<VoNotice> = DailyManage.getInstance().getVos(3) as Vector.<VoNotice>;
...
//				SecondsTimer.addFunction(onSecondTimer);
		...
		private static function actionShortCut( id:uint ):void
		{
			if( id < 100 )
			{
				var vo:VoDaily = DailyManage.getInstance().getVos(0)[id];
				vo.execute();
			}
			else 
			{
				var data:VoGuildAction = GuildManager.instance.actiondata[id - 100];
				MapSystem.mapTo.toMap( data.shortcutmap,data.shortcutx,data.shortcuty,true );
			}
		}
