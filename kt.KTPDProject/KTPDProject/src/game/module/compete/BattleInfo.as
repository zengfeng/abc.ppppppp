			...
			for (var i : int = VoCompete.record.length - 1;i > -1;i--)
			{
				...
				for each (var item:XML in xml["battledata"]["item"])
				{
					if (Number(item.@id) == VoCompete.record[i].result + 90)
					{
						...
						_nameString = StringUtils.addColorById(StringUtils.addLine(VoCompete.record[i].name),VoCompete.record[i].color+1);
		...
		public function addNewBattleInfo() : void
		{
			...
			for each (var item:XML in xml["battledata"]["item"])
			{
				if (Number(item.@id) == VoCompete.voNewBattle.result + 90)
				{
					...
					_nameString = StringUtils.addColorById(StringUtils.addLine(VoCompete.voNewBattle.name),VoCompete.voNewBattle.color+1);
					_nameString = StringUtils.addEvent(_nameString, _eventsring);
...
//			}
		...
		private function linkHandler(event : TextEvent) : void
		{
            ...
            if(sign=="zhan")
			{
				var id:int=int(event.text.slice(4));
				return;
			}
           else if(sign=="name")
		   {
