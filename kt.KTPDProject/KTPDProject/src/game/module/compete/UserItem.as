package game.module.compete
{
	...
	public class UserItem extends GPanel
	{
		...
		private var _hasUser:Boolean;
		private var _userHead : GImage ;
		private var userColor : int ;
		private var _isuser : Boolean;
		private var _rankTitleText : TextField;
...
//			_hasUser=hasuser;
		...
		private function addUserHeadPanel() : void
		{
			...
			userhead_bg = UIManager.getUI(new AssetData(UI.COMPETE_USERHEADBG));
		...
		private function onOver(event : MouseEvent) : void
		{
			...
			userhead_bg = UIManager.getUI(new AssetData(UI.COMPETE_USERHEADBG_OVER));
			userhead_bg.width = 92;
			userhead_bg.height = 91;
			userhead_bg.x = 4.5;
			userhead_bg.y = 13;
...
//				else
//				{
//						 StateManager.instance.checkMsg(107);
//				}
			}
			else
			{
...
//			_userHead.align=new GAlign(-3,-1,-1,80);
		...
		{
              _userNameText.htmlText=StringUtils.addColorById(StringUtils.addLine(_userName),_userColor+1);
		}
		...
		{
               _userNameText.htmlText=StringUtils.addColorById(_userName,_userColor+1);
		}
		...
		public function upDateUserItem(headid : int = 0, name : String = "", rank : String = "", level : int = 0, color : int = 0,isuser:Boolean=false,istooltip:Boolean=true,hasUser:Boolean=true) : void
		{
			//_isSelected=false;
			_isuser=isuser;
			...
			if (isuser == false)
			{
				_userNameText.htmlText = StringUtils.addColorById(name, color + 1);		
			}
			else
			{
			...
			// _userHeadPanel.toolTip.source = "<font color='#FF6633' size='16'>"+name+" "+level+"级</font>";
			// +rank + "\n每期排名奖励:\n1234声望，1234银币
		}
		...
		//	userhead_bigbg1.filters=[colorMatrixFilter_disable];
			...
			userhead_bigbg2 = UIManager.getUI(new AssetData(UI.COMPETE_MYUSERBG_LIGHT));
		...
		public function readString() : void
		{
			...
			{
				silverstr = (13000 + (31 - rank_int) * 500).toString();
				xiuweistr = "400";
			}
			else if (rank_int <= 100 && rank_int >= 31)
			{
				silverstr = (9500 + (101 - rank_int) * 500).toString();
				xiuweistr = "300";
			}
			else if (rank_int <= 500 && rank_int >= 101)
			{
				silverstr = (4500 + (501 - rank_int) * 500).toString();
				xiuweistr = "200";
			}
			else if (rank_int <= 1000 && rank_int >= 501)
			{
				silverstr = (2000 + (1001 - rank_int) * 500).toString();
				xiuweistr = "150";
			}
