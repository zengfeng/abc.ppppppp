package game.module.notification
{
	...
	public class ActionIcoButton extends GComponent
	{
		...
		public function setNotification(item : VoNotification) : void
		{
			...
			_item = item;
			if ((_item.typeId == 0 || _item.typeId == 1) && _item.value.paramNum.length > 0)
		...
		override protected function create() : void
		{
			...
			addChildAt(_back, 0);
			toolTip.source = _vo.getTips(_item.value.paramNum, _item.value.paramStr);
		}
		...
		private function openTarget() : void
		{
			...
			switch(_vo.openType)
			{
				...
				case 2:
					...
					break;
				case 3:
					Alert.show(_vo.getTips(_item.value.paramNum, _item.value.paramStr));
					NotificationProxy.delNotification(_item.id);
					break;
				case 4:
					Alert.show(_vo.getTips(_item.value.paramNum, _item.value.paramStr), "", Alert.OK | Alert.CANCEL, okFun);
					break;
				case 5:
					Alert.show(_vo.getTips(_item.value.paramNum, _item.value.paramStr));
					NotificationProxy.delNotification(_item.id);
				...
				case 9:
					{
						Alert.show(_vo.getTips(_item.value.paramNum, _item.value.paramStr),"",Alert.OK|Alert.CANCEL,
							function(type:String):Boolean{ 
								if( type == Alert.OK_EVENT ){ 
										var btn:IMenuButton = MenuManager.getInstance().changMenu(MenuType.TRADE);
										(btn.target as TradeView).switchPanel(1);
									}
									return true ;
								 });
					}
				...
				case 10:
					{
						Alert.show(_vo.getTips(_item.value.paramNum, _item.value.paramStr),"",Alert.OK|Alert.CANCEL,
							function(type:String):Boolean{ 
								if( type == Alert.OK_EVENT ){ 
										var btn:IMenuButton = MenuManager.getInstance().changMenu(MenuType.TRADE);
										(btn.target as TradeView).switchPanel(2);
									}
									return true ;
								 });
				...
				case 11:
					{
						Alert.show( _vo.getTips(_item.value.paramNum, _item.value.paramStr),null,Alert.OK|Alert.CANCEL, 
							function(type:String):Boolean{
								if( type == Alert.OK_EVENT ){
