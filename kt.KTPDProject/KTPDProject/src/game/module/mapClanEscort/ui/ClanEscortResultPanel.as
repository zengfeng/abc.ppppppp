package game.module.mapClanEscort.ui {
    ...
    public class ClanEscortResultPanel extends GComponent
    {
        ...
        public function ClanEscortResultPanel(singleton : Singleton)
        {
            singleton;
        ...
        /** 获取单例对像 */
        static public function get instance() : ClanEscortResultPanel
        {
            if (_instance == null)
            {
...
//        private var modalSkin : Sprite;
        ...
        public function initViews() : void
        {
			...
			bg = UICreateUtils.createSprite( UI.VIP_BG,380,405,5,6 );
			addChild(bg);
			
			bg = UICreateUtils.createSprite( UI.COMMON_BACKGROUND03,356,159,17,77 );
			addChild(bg);
			
//			bg = UICreateUtils.createSprite( UI.COMMON_BACKGROUND03,356,83,17,255 );
//			addChild(bg);
			
			bg = UICreateUtils.createSprite( GuildUtils.titleBar , 350,24,20,82 );
			addChild(bg);
			
			var txt:TextField = UICreateUtils.createTextField("排名",null,28,21,29,85,TextFormatUtils.key);
			txt.filters = [FilterUtils.defaultTextEdgeFilter];
			addChild(txt);
			
			txt = UICreateUtils.createTextField("族员",null,28,21,180,85,TextFormatUtils.key);
			txt.filters = [FilterUtils.defaultTextEdgeFilter];
			addChild(txt);
			
			txt = UICreateUtils.createTextField("造成伤害",null,52,21,302,85,TextFormatUtils.key);
			txt.filters = [FilterUtils.defaultTextEdgeFilter];
			addChild(txt);
			
			_winTitle = UIManager.getUI( new AssetData("escor_success","embedFont"));
			_winTitle.x = 9 ;
			_winTitle.y = 10 ;
...
//			_failTitle = UICreateUtils.createSprite( "escor_fail",372,68,7,8 );
			_failTitle = UIManager.getUI( new AssetData("escor_fail","embedFont"));
			_failTitle.x = 9 ;
			_failTitle.y = 10 ;
			addChild(_failTitle);
			...
			_membercnt = UICreateUtils.createTextField(null,null,170,21,200,240,TextFormatUtils.panelContentRight);
			addChild(_membercnt);
			
			bg = new FrameArea("获得奖励", 356, 83 );
			bg.x = 17 ;
			bg.y = 260 ;
			addChild(bg);
			
			_resultText1 = UICreateUtils.createTextField(null,null,200,22,27,296,TextFormatUtils.panelContent );
			addChild(_resultText1);

			_resultText2 = UICreateUtils.createTextField(null,null,200,22,27,316,TextFormatUtils.panelContent );
			addChild(_resultText2);

			_btnLeave = UICreateUtils.createGButton("离开",88,30,151,361,KTButtonData.SMALL_BUTTON);
			addChild(_btnLeave);
