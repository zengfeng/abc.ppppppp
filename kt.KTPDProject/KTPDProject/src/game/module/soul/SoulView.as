package game.module.soul
{
	...
	public class SoulView extends GCommonWindow  implements IModuleInferfaces, IAssets
	{
		...
		private function addSoulPower() : void
		{
			...
			_soulPowerText = UICreateUtils.createTextField(null, null, 120, 0, 55, 20, TextFormatUtils.panelContent);
			// _soulPowerText.x = 55;
			// _soulPowerText.y = 20;
		...
		{
			// _subPanel.addChild(UICreateUtils.createTextField("元神列表", null, 0, 0, 354, 2, TextFormatUtils.panelSubTitle));
			_subPanel.addChild(UICreateUtils.createTextField("←拖动至元神位可装备，拖离可拆卸", null, 240, 0, 354, 2, TextFormatUtils.panelContent));
		}
		...
		private function addCheckBox() : void
		{
			...
			// var text : TextField = UICreateUtils.createTextField("只显示绑定", null, 160, 0, 566, 23, TextFormatUtils.panelContent);
			_subPanel.addChild(_checkBox);
			// _subPanel.addChild(text);
		}
		...
		private function addPack() : void
		{
			...
			// _emptyText = UICreateUtils.createTextField("没有符合条件的元神", null, _soulGrid.width, 20, 0, _soulGrid.height * 0.5 - 10, TextFormatUtils.panelSubTitleCenter);
			// _emptyText.visible = false;
			// _soulGrid.addChild(_emptyText);
		...
		{
			return [new LibData(VersionManager.instance.getUrl("assets/swf/soul_flame.swf"), "soul_flame")];
		}
