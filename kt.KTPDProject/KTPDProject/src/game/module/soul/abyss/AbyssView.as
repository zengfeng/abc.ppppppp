package game.module.soul.abyss {
	 ...
	 * @author jian
	 */
	public class AbyssView extends GPanel implements IModuleInferfaces, IAssets
	{
		// =========================================================================
		// 定义
		// =========================================================================
		public static const GOLD_SUMMON_VIP_LEVEL : int = 3;
		public static const SILVER_SUMMON_COST : uint = 2000;
		public static const GOLD_SUMMON_COST : uint = 200;
//		public static const BOSS_AVATAR_IDS : Array = ["67113879", "67113880", "67113881", "67113882"];
		public static const BOSS_AVATAR_IDS : Array = ["5015", "5016", "5017", "5018"];
		public static const BOSS_NAMES : Array = ["百年妖兽", "千年妖兽", "万年妖兽", "洪荒妖兽"];
		private static const HERO_POSITION : Point = new Point(825, 480);
		private static const BOSS_POSITION : Point = new Point(580, 305);
		private static const FIGHT_POSITION : Point = new Point(650, 342);
		// =========================================================================
		// 属性
		// =========================================================================
		private var _silverSummonLeft : uint = 0;
		private var _goldSummonLeft : uint = 0;
		private var _silverSummonCount : uint = 0;
		private var _goldSummonCount : uint = 0;
		...
		private function addSummonButtons() : void
		{
			...
			_silverSummonLabel = UICreateUtils.createGradientLabel({width:100, height:25, x:(_silverSummonButton.width - 100) * 0.5 - 3, y:_silverSummonButton.height - 15, gradientAsset:new AssetData("GGradientLabel_skin"), textFormat:TextFormatUtils.graphicLabel, textFieldFilters:[FilterUtils.defaultTextEdgeFilter]});
			...
			_goldSummonLabel = UICreateUtils.createGradientLabel({width:100, height:25, x:(_goldSummonButton.width - 100) * 0.5 - 27, y:_goldSummonButton.height - 20, gradientAsset:new AssetData("GGradientLabel_skin"), textFormat:TextFormatUtils.graphicLabel, textFieldFilters:[FilterUtils.defaultTextEdgeFilter]});
		...
		private function getSilverSummonTip() : String
		{
			if (_silverSummonLeft == 0)
				...
				return "花费" + StringUtils.addColor(SILVER_SUMMON_COST * _silverSummonCount + "银币", ColorUtils.HIGHLIGHT_DARK) + "随机召唤" + StringUtils.addColor(_silverSummonCount + "次", ColorUtils.HIGHLIGHT_DARK) + "妖兽，今日还可召唤" + StringUtils.addColor(_silverSummonLeft + "次", ColorUtils.HIGHLIGHT_DARK);
		}
		...
		private function getGoldSummonTip() : String
		{
			if (_goldSummonLeft == 0)
				...
				return "花费" + StringUtils.addColor(GOLD_SUMMON_COST * _goldSummonCount + "元宝", ColorUtils.HIGHLIGHT_DARK) + "指定召唤" + StringUtils.addColor(_goldSummonCount + "次", ColorUtils.HIGHLIGHT_DARK) + "洪荒妖兽，今日还可召唤" + StringUtils.addColor(_goldSummonLeft + "次", ColorUtils.HIGHLIGHT_DARK);
		}
		...
		private function addOtherButtons() : void
		{
			...
			exitButtonData.align = new GAlign(-1, 0, 80);
		...
		private function addNameBoard() : void
		{
			// var nameBoardText : Sprite = UIManager.getUI(new AssetData(UI.ABYSS_NAME_BOARD_TEXT));
			// nameBoardText.x = 33;

			_nameBoard = UIManager.getUI(new AssetData(UI.NAME_BOARD));
			var nameBoardText : TextField = UICreateUtils.createTextField("黄泉深渊", null, 100, 30, 65, 7, TextFormatUtils.windowTitle);
			nameBoardText.filters = [new DropShadowFilter(0, 45, 0x000000, 1, 5, 5, 2)];
			_nameBoard.addChild(nameBoardText);
			addChild(_nameBoard);
		}
		...
		{
			_heroAvatar = AvatarManager.instance.getAvatar(UserData.instance.myHero.id, AvatarType.PLAYER_BATT_BACK) as AvatarPlayer;
			_heroAvatar.mouseEnabled = false;
			_heroAvatar.mouseChildren = false;
			_heroAvatar.setName(UserData.instance.playerName, "#FFFFFF");
			_heroAvatar.showName();
			_heroAvatar.setAction(AvatarManager.BT_STAND);

			updateHeroPosition();

			_screenLayer.addChild(_heroAvatar);
		}
		...
		override protected function onShow() : void
		{
			...
			if (_heroAvatar)
				_heroAvatar.setAction(AvatarManager.BT_STAND);
			ToolTipManager.instance.registerToolTip(_silverSummonButton, ToolTip, getSilverSummonTip);
			ToolTipManager.instance.registerToolTip(_goldSummonButton, ToolTip, getGoldSummonTip);
		...
		private function silverSummonBossHandler(event : MouseEvent) : void
		{
			...
			if (_batchCKBox.selected)
			{
				if (UserData.instance.trySpendSilver(SILVER_SUMMON_COST * _silverSummonCount) >= 0)
				{
			...
			{
				if (UserData.instance.trySpendSilver(SILVER_SUMMON_COST) >= 0)
				{
					GuideMange.getInstance().checkGuideByMenuid(MenuType.SOUL_ABYSS, 2, _lootButton);
		...
		private function goldSummonBossHandler(event : MouseEvent) : void
		{
			...
			if (_batchCKBox.selected)
			{
				if (UserData.instance.trySpendTotalGold(GOLD_SUMMON_COST * _goldSummonCount) >= 0)
				{
			...
			{
				if (UserData.instance.trySpendTotalGold(GOLD_SUMMON_COST) >= 0)
				{
		...
		private function bossAppear() : void
		{
			...
			appearMc.y = BOSS_POSITION.y;
			_screenLayer.addChild(appearMc);
			appearMc.addEventListener(Event.COMPLETE, bossAppearComplete);
		 ...
		 * Boss出现完成
		 */
		private function bossAppearComplete(event : Event) : void
		{
			(event.target as MovieClip).removeEventListener(Event.COMPLETE, bossAppearComplete);
		...
		{
			TweenLite.to(_heroAvatar, 0.5, {x:FIGHT_POSITION.x, y:FIGHT_POSITION.y, onComplete:function() : void
			{
				setTimeout(heroAttack, 150);
			}});
		}
		...
		private function heroAttackComplete(event : Event) : void
		{
			...
			_heroAvatar.setAction(AvatarManager.BT_STAND);
			TweenLite.to(_heroAvatar, 0.5, {x:HERO_POSITION.x, y:HERO_POSITION.y});
			bossDown();
		}
		...
		{
			_bossAvatar.setAction(AvatarManager.DIE);
			_bossAvatar.hideShodow();
			setTimeout(soulFallOut, 200);
		}
		...
		private function soulFallOut() : void
		{
			...
			for (var i : uint = 0; i < len; i++)
			{
				...
				award.x = BOSS_POSITION.x - 30;
				award.y = BOSS_POSITION.y - 50;
				award.arrived = false;
				...
				TweenMax.to(award, 1.4 + 0.1 * i, {delay:0.1 * i, bezierThrough:[{x:thru1.x, y:thru1.y}, {x:thru2.x, y:thru2.y}, {x:to.x, y:to.y}], ease:Sine.easeIn, onComplete:onSoulArrive, onCompleteParams:[award]});
		...
		private function lootAwardHandler(event : MouseEvent) : void
		{
			...
			if (UserData.instance.tryPutPack(_fightAwards.length) >= 0)
				sendAbyssLootAll();
		}
		...
		{
			var vanishMC : MovieClip = RESManager.getMC(new AssetData(UI.GOLD_VANISH));
		...
		{
			return [new LibData(VersionManager.instance.getUrl("assets/swf/abyss.swf"), "abyss")];
		}
