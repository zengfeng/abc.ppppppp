package game.module.guild.action {
	...
	import com.commUI.icon.CommonIcon;
	...
	public class GuildActionItem extends GPanel
	{
		...
		private static const ACTION_CARVEN:int = 1 ;
		private static const ACTION_DRINKTEA:int = 2 ;
		private static const ACTION_ESCOR:int = 3 ;
		
		protected var _icon : CommonIcon ;
...
//		private var _finalLabel : TextField ;
		...
		{
			var bg : Sprite = UIManager.getUI((line&1) == 0 ? new AssetData(GuildUtils.clanLightBar):new AssetData(GuildUtils.clanDeepBar));
			bg.width = width ;
			bg.height = height ;
			addChild(bg);
			...
			_icon.background = UIManager.getUI(new AssetData(UI.ROUND_ICON_BACKGROUND_GREEN));
			addChild(_icon);
...
//			fmt.color = ColorUtils.PANELTEXT0X ;
			_nameLabel = UICreateUtils.createTextField(null,null,65,20,67,26,fmt);
//			_nameLabel.width = 65 ;
...
//			fmt.color = ColorUtils.PANELTEXT0X ;
			_infoLabel = UICreateUtils.createTextField(null,null,110,18,160,27,fmt);
...
//			}
		...
		protected function sendGuildShortcut(fun:Function = null,arg:Array = null):void
		{
			if( _gadata != null )
			{
				MapSystem.mapTo.toMap( _gadata.shortcutmap,_gadata.shortcutx,_gadata.shortcuty,true , fun, arg );
			}
		}
