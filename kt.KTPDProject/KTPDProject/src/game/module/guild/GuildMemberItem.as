...
//			fmt.color =  ColorUtils.PANELTEXT0X;
			_name = UICreateUtils.createTextField( null, null, 115,25,0,2 , fmt );
			_name.selectable = false ;
			_name.mouseEnabled = true ;
			_name.autoSize = "center";
			addChild(_name);
			
			_level = UICreateUtils.createTextField( null, null, 55 ,25 ,115, 2 , fmt );
			_level.selectable = false ;
			addChild(_level);
			
			_post = UICreateUtils.createTextField( null, null, 40 ,25 ,170, 2 , fmt );
			_post.selectable = false ;
			addChild(_post);
			
			_rank = UICreateUtils.createTextField( null, null, 75 ,25 ,210, 2 , fmt );
			_rank.selectable = false ;
			addChild(_rank);
			
			_devote = UICreateUtils.createTextField( null, null, 65 ,25 ,285, 2 , fmt );
			_devote.selectable = false ;
			addChild(_devote);
			
			_login = UICreateUtils.createTextField( null, null, 88 ,25 ,348, 2 , fmt );
		...
		private function addBg() : void {
			...
			_deepbg = UICreateUtils.createSprite(GuildUtils.clanDeepBar,426,25);
			_lightbg = UICreateUtils.createSprite(GuildUtils.clanLightBar,426,25);
			addChild(_deepbg);
		}
		...
		public function set vo( value:VoGuildMember ):void{
			...
			else 
			{
				...
				_login.text = getTimeOpposite(_vo.latestonl);
