package game.module.recruitHero.highFindHero
{
	...
	public class FindHeroPanel extends GComponent
	{
		...
		{
			_mcMagicMirror = RESManager.getMC(new AssetData("mojing", "embedFont"));
		...
		private function addButtons() : void
		{
			...
			buttonData.upAsset = new AssetData(UI.HIGH_FINDHERO_BTNGOLD_UP);
			buttonData.overAsset = new AssetData(UI.HIGH_FINDHERO_BTNGOLD_OVER);
			buttonData.downAsset = new AssetData(UI.HIGH_FINDHERO_BTNGOLD_DOWN);
			...
			var str : String = "<font size='12',color='#ffffff'>请神需花费50元宝</font>";
			...
			buttonData.upAsset = new AssetData(UI.HIGH_FINDHERO_BTN_UP);
			buttonData.overAsset = new AssetData(UI.HIGH_FINDHERO_BTN_OVER);
			buttonData.downAsset = new AssetData(UI.HIGH_FINDHERO_BTN_DOWN);
			buttonData.disabledAsset = new AssetData("EnterButtonSkin_Disable");
			buttonData.width = 96;
			buttonData.height = 39;
			buttonData.x = 85;
...
//			_mcLight.x = mcX - 5;
//			_mcLight.y = mcY + 104;
			_mcLast = RESManager.getMC(new AssetData("mcLast", "embedFont"));
			_mcLast.x = mcX;
			_mcLast.y = mcY;
			_mcGround = RESManager.getMC(new AssetData("mcGround", "embedFont"));
			_mcGround.x = mcX;
			_mcGround.y = mcY;
			_mcWealth = RESManager.getMC(new AssetData("mcWealth", "embedFont"));
			_mcWealth.x = mcX;
			_mcWealth.y = mcY;
			_mcBlessing = RESManager.getMC(new AssetData("mcBlessing", "embedFont"));
			_mcBlessing.x = mcX;
			_mcBlessing.y = mcY;
			_mcOpen = RESManager.getMC(new AssetData("openFhero", "embedFont"));
			_mcOpen.x = mcX;
			_mcOpen.y = mcY;
			_mcCurrent = _mcOpen;
			addChild(_mcCurrent);
...
//			_mcLight.play();
		...
		private function onFindGoldBtnClick(event : MouseEvent) : void
		{
			...
			StateManager.instance.checkMsg(311, [50], okFun);
		}
