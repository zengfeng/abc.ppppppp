package game.module.recruitHero.highFindHero
{
	 ...
	 * @author zheng
	 */
	public class HighFindHeroView extends GCommonWindow
	{
		
        // ===========================================================
		// @定义
		// ===========================================================	
		private static const Stone:int=3101;    //龙纹
		private static const Token:int=3102; 	//七星
		// ===========================================================
		// @属性
		// ===========================================================
		...
		private function addBg():void
		{
			...
			var findbg:Sprite=UIManager.getUI(new AssetData(UI.HIGH_FINDHERO_BACKROUND));
			findbg.width = 622;
			findbg.height = 322;
			findbg.x = 15;
			findbg.y = 88;
		    ...
		    _blackbg1=UIManager.getUI(new AssetData(UI.HIGH_FINDHERO_BLACKBG));
			_blackbg1.width = 126;
			_blackbg1.height = 42;
			_blackbg1.x = 27;
			_blackbg1.y = 99;
			...
			_blackbg2=UIManager.getUI(new AssetData(UI.HIGH_FINDHERO_BLACKBG));
			_blackbg2.width = 126;
			_blackbg2.height = 42;
			_blackbg2.x = 27;
			_blackbg2.y = 152;
		...
		private function addLabels():void
		{
			...
			_textRule=UICreateUtils.createTextField(null, str, 80, 18, 560, 100, UIManager.getTextFormat());
			_textRule.mouseEnabled=true;
			this.contentPanel.addChild(_textRule);
			
			_textRemain=UICreateUtils.createTextField("今日免费请神次数： ", null, 110, 18, 420, 360, UIManager.getTextFormat(12,0xffffff));
			this.contentPanel.addChild(_textRemain);
			
			_remainNum=UICreateUtils.createTextField("", null, 20, 18, 530, 360, UIManager.getTextFormat(12,0xffffff));
			this.contentPanel.addChild(_remainNum);
			
			_textGold=UICreateUtils.createTextField("元宝请神，大大提高福神出现概率", null, 200, 18, 420, 360, UIManager.getTextFormat(12,0xffffff));
...
//			this.contentPanel.addChild(_goldTextScroll);
		...
		private function onRefreshPanel(msg:SCHeroBet):void
		{
			...
			var nums:int=msg.item<<16>>16;
			var itemid:int=msg.item>>16;
			_findHeroPanel.refreshGodMC(godId);
			_findHeroPanel.refreshBtn(times);
			
			onScrollBar(itemid,nums,godId);
...
//			TweenLite.to(_goldTextScroll, 1, {y:230, alpha:1,overwrite:1,ease:Sine.easeOut, onComplete:onRemoveGold});
		...
		private function tabDataHeroBox() : GTabData
		{
			if (!_tabDataHeroBox)
			{
				...
				data.overAsset = new AssetData(UI.BUTTON_BOXTAB_UNSEL_UP);
				data.disabledAsset = new AssetData(SkinStyle.emptySkin);
				data.selectedUpAsset = new AssetData(UI.BUTTON_BOXTAB_UNSEL_UP);
