package game.module.role {
	...
	public class CreateRole extends Sprite
	{
		...
		private function ElementLoader_func(evt : Event) : void
		{
			...
			PlayButton.x = (UIManager.stage.stageWidth - 380) / 2+110;
			PlayButton.y =(UIManager.stage.stageHeight - 540) / 2+464;
			...
			sprite.addEventListener(MouseEvent.CLICK,pushValue);
		...
		private function TipsEvent_func(evt : MouseEvent) : void
		{
			...
			{
				TipsText.htmlText = "天师（男）：擅长仙术攻击<br><font color='#FFCC00'>拥有最高的仙攻成长，使用天蝉宝杖。</font>";
				_Height = 46;
			}
			else if (evt.target == Button6)
			{
				TipsText.htmlText = "天师（女）：擅长仙术攻击<br><font color='#FFCC00'>拥有最高的仙攻成长，使用天蝉宝杖。</font>";
				_Height = 46;
			}
