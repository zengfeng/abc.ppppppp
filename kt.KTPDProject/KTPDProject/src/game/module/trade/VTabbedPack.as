...
//			gridData.showEmptyText = "没有非绑定的物品可进行交易";
		...
		private function updateVTabList() : void
		{
			if (UserData.instance.level >= 70)
				_tabList.source = [new LabelSource("装备", Item.EQ), new LabelSource("消耗品", Item.EXPEND), new LabelSource("元神", Item.SOUL), new LabelSource("灵珠", Item.GEM), new LabelSource("材料", Item.ENHANCE|Item.JEWEL)];
			else
				_tabList.source = [new LabelSource("装备", Item.EQ), new LabelSource("消耗品", Item.EXPEND), new LabelSource("元神", Item.SOUL), new LabelSource("材料", Item.ENHANCE|Item.JEWEL)];
		}
