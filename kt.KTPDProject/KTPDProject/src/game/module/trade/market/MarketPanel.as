package game.module.trade.market
{
	...
	public class MarketPanel extends GPanel
	{
		...
		private var _bigNode : Vector.<TreeNode> = new Vector.<TreeNode>();
		private var tree_id : uint;
		...
		private function addTreeNode() : void
		{
			...
			addNodeData(_root, "普通", _bigNode, 10);
			...
			_childPt = _tree.model.getRoot().getChildren()[0];
			addNodeData(_childPt, "节日", _commonNode, 11);
			addNodeData(_childPt, "碎片", _commonNode, 12);
			addNodeData(_childPt, "消耗品", _commonNode, 13);

			_childZb = _tree.model.getRoot().getChildren()[1];
			...
			_childYs = _tree.model.getRoot().getChildren()[2];
        ...
        /*创建原石子节点*/        
		private function addSoulNodes() : void
		{
			addNodeData(_root, "原石", _bigNode, 60);
			_childLz = _tree.model.getRoot().getChildren()[5];
...
//				var searchResult : Array = new Array();
				...
				var uniqItems : Object = {};                      //创建Object对象

				for each (var conf:ItemConfig in ItemConfigManager.instance.configDict)   //遍历ItemConfig类
				{
					// 猥琐方法获取搜索名称
				...
				for each (var result:Item in uniqItems)                  //对uniqItems进行遍历 为字体加颜色并加入label数组
				{
		...
		override protected function onShow() : void
		{
			...
			Common.game_server.addCallback(0xFFF1, onUserDataChangeUp);
		...
		//	tree_id=0;
			...
			Common.game_server.removeCallback(0xFFF1, onUserDataChangeUp);
		}
/************************搜索框操作函数********************************/
        /* 用户的搜索框输入事件 重新搜索*/
	 ...
	 /************************* 排序按钮操作函数***************************/
      ...
      /************************* 排序按钮操作函数***************************/
        ...
        /*用户超过70级要刷新树 加入元神节点*/
		private function onUserDataChangeUp(msg : CCUserDataChangeUp) : void
		{
			if (msg.level >= 70 && msg.oldLevel < 70)
			{
				addSoulNodes();
				refreshTree();
			}
		...
		private function tree_singleClickHandler(event : Event) : void
		{
			sortClear();
			_comboBox.textInput.text = "";
			tree_id = (event.target as MarketPropCell).getData1();
