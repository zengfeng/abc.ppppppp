package game.module.battle.view
{
	...
	public class BTSystem
	{
		...
		//		//  地方点坐标
		// public static const EnemyPoints:Array = [
		// [707,257],[639,291],[572,325],[504,359],[436,393],
		// [640,223],[572,257],[504,291],[436,325],[368,359],
		// [571,189],[504,223],[436,257],[368,291],[300,325],
		// [504,155],[436,189],[368,223],[300,257],[232,291],
		// [436,121],[368,155],[300,189],[232,223],[164,257]
		// ];
		// 资源部分---------------------------------------------------
		// 普通效果
...
//		public static var EffectDownwords : Class;
		...
		public var PLAYMODLE:uint = 0;     //0:正常战斗  1:录像
		...
		public var isAvatarLoad:Boolean = false;   //avtar已经加入场景
		
		private var BaseOffsetX:int = 0;   //基础x像素偏移
			
		private var BaseOffsetY:int = 0;   //基础y像素偏移
		
		private var BackGroundPic:Bitmap;  //战斗背景
		...
		private var PlayerRoleID:int;       //自己头像id
		private var PlayerRoleName:String;  //我名字
		private var PlayerLevel:int;
		private var PlayerColor:uint;       //潜力
		private var EnemyRoleID:int;        //对方头像id
		private var EnemyRoleName:String;   //对方名字
...
//		private var SWFCONTAINER:Vector.<MovieClip> = new Vector.<BtInit>();
		...
		public function saveBttleData() : void
		{
			...
			//trace(BtBuffProcess.data.length);
		...
		private function launch() : void
		{
			...
			if (BTSystem.INSTANCE().getFightTypeID() == BTSystem.FT_DUGEON)  // 副本
			{
				...
				{
					BackGroundPic = new Bitmap(MapSystem.printScreen(mapD.pointX, mapD.pointY));
					...
					// //trace(StaticConfig.cdnRoot + AttackData.mapidlist[mapID]);
				}
		...
		private function Resize_func(evt : Event) : void
		{
			...
			if(userpanel)
			{
				...
				userpanel.y = int(UIManager.root.stage.stageHeight-userpanel.height-32);
			}
		...
		{
			return (_fightType >> 8) & 0xFFFF;
		}

		public function getOverType() : uint   // 结束面板类型
		{
			return (_fightType & 0xFF );
		}
		...
		public function start() : void
		{
			...
			if(!isAvatarLoad)
			{
				...
				for (i = 0; i < length; i++)
				{
					var self : BTAvatar = new BTAvatar(BtProcess.selfList[i] as FighterInfo);
				...
				for (i = 0; i < length; i++)
				{
					var enemy : BTAvatar = new BTAvatar(BtProcess.enemyList[i] as FighterInfo);
...
//			sendEnd();
		...
		{
			clearBattlePanel();
//			if (BackGroundPic && BackGroundPic.bitmapData)
//				BackGroundPic.bitmapData.dispose();
			if (PLAYMODLE == 0)
			{
				if (_container)
					_container.hide();
			...
			{
				var obj : * = _battleSprite.getChildAt(0);
				if (obj is BTAvatar)
				{
		...
		public function DisposAllResources():void
		{
			...
			{
				RESManager.instance.remove(RESOURCES[i]);
			}
		...
		public function endBattleDelay() : void
		{
			setTimeout(end, 6000);
		}
		...
		// 技能buff效果，被攻击者播放
		private function PlaySkillBuff(target : BTAvatar, skillId : uint, control : uint = 0) : void
		{
			target.playSkillBuff(skillId, control);
		}
...
//		}
		...
		private function Fight_func(p : BtProcess) : void
		{
			...
			if (p == null)
			{
				...
				// end();
				var _resdata : resultData = new resultData(earnExp, 0, rewardlist);
				if (!_btResult)
				{
					_btResult = new BTResult();
					_btResult.setTextResult(getFightTypeID(), b_Result ? 1 : 0, _resdata)
				}
				else
				{
					_btResult.setTextResult(getFightTypeID(), b_Result ? 1 : 0, _resdata);
				...
				_btResult.show();
				// _btResult.scaleX = 0.2;
				// .scaleY = 0.2;
				var tx : int = _btResult.x;
				var ty : int = _btResult.y;
				_btResult.x = (UIManager.root.stage.stageWidth - _btResult.width) / 2;
				_btResult.y = (UIManager.root.stage.stageHeight - _btResult.height) / 2 - 40;
				_btResult.alpha = 0;
			...
			if ( p.oneAtkInfo == null)   // 效果状态改变
			{
				...
				if (tgDmg > 0)
				{
					...
					TweenLite.to(target, 0, {delay:target.getAtkTimeTrad(), onComplete:PlaySkillBuff, onCompleteParams:[target, skillid, showtype], overwrite:0});
					BaseDelay += target.getAtkTimeTrad();
				}
				else // 判断是否取消眩晕
				{
					TweenLite.to(target, 0, {delay:0.2, onComplete:PlaySkillBuff, onCompleteParams:[target, skillid, showtype], overwrite:0})
				}
			...
			else if (p.oneAtkInfo != null && p.defendList != null)
			{
				...
				for (i = 0; i < p.SChangeList.length; ++i)
				{
					if(p.SChangeList[i].skillid == skillType.GAUGEFULL && p.SChangeList[i].type == 0)
						self.playSkillBuff(skillType.GAUGEFULL, 0);	
				}
		...
		public function FightStepAOurs(self : BTAvatar, p : BtProcess):void
		{
			...
			if(self.theFighterInfo.getSpellEftId2() > 0 && self.theFighterInfo.getBuffID2() > 0)  //如果有技能效果动画
			{
				...
				if(self.theFighterInfo.getTargetType2() == 1)  //技能出手方表现
				{
					...
					addhp = rescuer.addHp;
					afteraddhp = rescuer.leftHp;//((self.theFighterInfo.getNowHp() + addhp) > self.getInitHP()) ? self.getInitHP(): (self.theFighterInfo.getNowHp() + addhp);
					...
					if (p.SChangeList[i].atkorBackatk == 0)
					{
						...
						if (p.SChangeList[i].letDie != 2)
						{
							...
							{}
								...
								effectAvatar.playSkillBuff(p.SChangeList[i].skillid, p.SChangeList[i].type);
							
							// 这个地方有问题。。。
							effectAvatar.playUpDownWords(p.SChangeList[i].skillid, p.SChangeList[i].type);
				...
				//	if(bskillAction) //有美术技能效果，等技能效果播放完了，继续下个回合
					{
						if (bskillAtk)
						{
							...
							var skill1waittime:Number = 0;   //效果1时间
							var skill2waittime:Number = 0;   //地面效果时间
							var skill3waittime:Number = 0;   //给己方加的效果时间
							...
							if(waittime > TestTime)
							   TestTime = waittime+0.5;//- tgAvatar.getAtkTimeTrad();
		...
		//		//  战斗
		// public function BTSystem(skillnum : uint)
		// {
		// allskillNum = skillnum;
		// for (var i : int = 0; i < allskillNum; i++)
		// {
		// var newclass : Class = new Class();
		// Effect_SkillVec.push(newclass);
		// }
		// }
	}
}