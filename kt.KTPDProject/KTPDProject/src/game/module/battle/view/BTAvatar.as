package game.module.battle.view {
	...
	public class BTAvatar extends AvatarFight {
		...
		public function playSkillEfft(playType : uint, spellEftid : uint, frontOrback : uint, needTurnOver : Boolean, angle : uint) : void {
			this.playSkill(spellEftid, playType, frontOrback, needTurnOver, angle, theFighterInfo.side);
		}

		public function playUpDownWords(skillid : uint, control : uint) : void {
			this.playupdownwords(skillid, control);
		}

		// control：0 消失 1 播放
		public function playSkillBuff(skillid : uint, control : uint) : void {
			this.playBuff(skillid, control);
		}
