package game.net.data.StoC {
	...
	public dynamic final class BossPlayerState extends com.protobuf.Message {
		 /**
		  *@playerid   playerid
		  **/
		...
		override public final function writeToBuffer(output:com.protobuf.WritingBuffer):void {
			...
			if (hasFlag) {
				...
				com.protobuf.WriteUtils.write$TYPE_UINT32(output, flag$field);
			}
		...
		override public final function readFromSlice(input:flash.utils.IDataInput, bytesAfterSlice:uint):void {
			...
			while (input.bytesAvailable > bytesAfterSlice) {
				...
				case 1:
					if (playerid$count != 0) {
						throw new flash.errors.IOError('Bad data format: BossPlayerState.playerid cannot be set twice.');
					}
					++playerid$count;
				...
				case 2:
					if (state$count != 0) {
						throw new flash.errors.IOError('Bad data format: BossPlayerState.state cannot be set twice.');
					}
					++state$count;
				...
				case 3:
					if (lefttime$count != 0) {
						throw new flash.errors.IOError('Bad data format: BossPlayerState.lefttime cannot be set twice.');
					}
					++lefttime$count;
				...
				case 4:
					if (goldnum$count != 0) {
						throw new flash.errors.IOError('Bad data format: BossPlayerState.goldnum cannot be set twice.');
					}
					++goldnum$count;
				...
				case 5:
					if (flag$count != 0) {
						throw new flash.errors.IOError('Bad data format: BossPlayerState.flag cannot be set twice.');
					}
					++flag$count;
