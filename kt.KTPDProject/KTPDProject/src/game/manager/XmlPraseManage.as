package game.manager {
	...
	import game.core.user.VoFunction;
	...
	import game.module.daily.VoCopy;
	import game.module.daily.VoDaily;
	import game.module.daily.VoNotice;
	import game.module.daily.transanim.KeyFrameAnimManager;
	import game.module.daily.transanim.TransKeyFrame;
	...
	import maps.configs.ParseMapConfig;
	...
	public final class XmlPraseManage
	{
		...
		/** 解析副本配置文件 */
		private static function parseDupl() : void
		{
			var duplMapData : DuplMapData = DuplMapData.instance;
			tempArray = (RSSManager.getInstance().getData("dungeonInfo", RSSManager.TYPE_CSV) as CSV).getData();
			if (!tempArray) return;
			duplMapData.parseDungeonInfoCSV(tempArray);
			RSSManager.getInstance().deleteData("dungeonInfo", RSSManager.TYPE_CSV);

			tempArray = (RSSManager.getInstance().getData("dungeon", RSSManager.TYPE_CSV) as CSV).getData();
			if (!tempArray) return;
			duplMapData.parseDungeonCSV(tempArray);
			RSSManager.getInstance().deleteData("dungeon", RSSManager.TYPE_CSV);
		}

		/** 解析副本配置文件 */
		private static function parseDuplLayerTipXML() : void
		{
			var duplMapData : DuplMapData = DuplMapData.instance;
			var xml : XML = RSSManager.getInstance().getData("dungeonLayerTip");
			if (!xml) return;
			duplMapData.parseDungeonLayerTipXML(xml);
			RSSManager.getInstance().deleteData("dungeonLayerTip");
		}
		...
		{
			var mapModel : MapModel = MapModel.instance;
			var mapXmlDic : Dictionary = RSSManager.getInstance().mapDic;
			for each (var map:XML in mapXmlDic)
			{
				...
				mapStruct.assetsMapId = int(mapXml.attribute("assetsMapId")) ;
				mapStruct.assetsMapId = mapStruct.assetsMapId ? mapStruct.assetsMapId : mapStruct.id ;
				mapStruct.name = mapXml.attribute("name");
				mapStruct.mapWH = new Point(int(mapXml.attribute("width")), int(mapXml.attribute("height")));
				mapStruct.mapWH.x = int(mapStruct.mapWH.x / 256) * 256;
				mapStruct.mapWH.y = int(mapStruct.mapWH.y / 256) * 256;

				mapStruct.singlePieceHW.x = 256;
				mapStruct.singlePieceHW.y = 256;
				mapStruct.pieceRepeatWH = new Point(mapStruct.mapWH.x / mapStruct.singlePieceHW.x, mapStruct.mapWH.y / mapStruct.singlePieceHW.y);
				mapStruct.distantFile = mapXml.attribute("distant");
				...
				for (var i : int = 0; i < npcXml.length(); i++)
				{
					...
					npcStruct.id = npcXml[i].@id;
					voNpc = RSSManager.getInstance().getNpcById(Number(npcStruct.id));
					if(mapStruct.id == 202)
					{
						trace(npcStruct.id , voNpc);
					}
					if (!voNpc) continue;
					voNpc.mapId = mapStruct.id;
					voNpc.x = npcXml[i].@x;
					voNpc.y = npcXml[i].@y;
					npcStruct.x = npcXml[i].@x;
					npcStruct.y = npcXml[i].@y;
					npcStruct.moveRadius = parseInt(npcXml[i].@moveRadius);
					npcStruct.attackRadius = parseInt(npcXml[i].@attackRadius);
					npcStruct.passColor = parseInt(npcXml[i].@passColor);
				...
				for (i = 0; i < gateXml.length(); i++)
				{
					...
					gateStruct.id = parseInt(gateXml[i].@id);
					gateStruct.position = new Point(gateXml[i].@x, gateXml[i].@y);
					gateStruct.standPosition = new Point(gateXml[i].@standX, gateXml[i].@standY);
				...
				mapModel.mapDic[mapStruct.id] = mapStruct;
			}
		}
		...
		/**  解析vip配置文件 */
		private static function parseVipXml() : void
		{
		...
		private static function parseFunction() : void
		{
			...
			for each (var arr:Array in tempArray)
			{
				var vo : VoFunction = new VoFunction();
				vo.parse(arr);
				StateManager.instance.initFunctionVo(vo);
			}
...
//                ClanManager.trends[vo.trendId] = vo ;
		...
		private static function parseDaily() : void
		{
			...
			for each ( item in xml["copy"]["item"])
			{
				var vo1 : VoCopy = new VoCopy();
				vo1.pares(item);
				DailyManage.getInstance().initCopyVo(vo1);
			}
