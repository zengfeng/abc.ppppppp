package com.commUI.icon
{
	...
	public class CommonIcon extends GComponent
	{
		 ...
		 * 设置堆叠个数
		 */
		public function set pileNums(pileNums : uint) : void
		{
			if (!_pileNumText)
			{
			...
			var text : String = "";
			if (pileNums == 0)
				text = StringUtils.addColor(pileNums.toString(), "#ff0000");
			else if (pileNums > 99)
				text = "99+";
			else
				text = pileNums.toString();
			_pileNumText.htmlText = StringUtils.addColorById(text, 1);
		}
		...
		public function clear() : void
		{
			_iconBitmap.clearUp();
			this.border = null;
			if (_tradeIcon) _tradeIcon.visible = false;
