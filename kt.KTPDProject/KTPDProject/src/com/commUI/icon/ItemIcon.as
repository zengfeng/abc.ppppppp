	...
	public class ItemIcon extends CommonIcon
	{
		...
		override public function set enabled(value : Boolean) : void
		{
			...
			if (value)
				...
				filters = [FilterUtils.disableFilter(0.4)];
		}
		...
		override protected function create() : void
		{
			...
			{
				tradeIcon = UIManager.getUI(new AssetData(UI.ICON_TRADABLE));
				_tradeIcon.visible = false;
			}
		...
		{
			if (thisdata.showBinding)
				_tradeIcon.visible = ((_source as Item).binding == false);
			else if (_tradeIcon)
				_tradeIcon.visible = false;
		}
		...
		{
			if (thisdata.showNums && _source && (_source as Item).config.stackLimit > 1 && !(_source is IUnique))
				pileNums = (_source as Item).nums;
			else
			{
