package maps
{
	...
	import maps.configs.MapConfig;
	import maps.configs.MapId;
    ...
    public class MapUtil
    {
        ...
        {
            var toPostion : Point;
            toPostion = MapFacade.landLayer.globalToLocal(postion);
            if (replace)
            {
        ...
        {
            var toPostion : Point;
            toPostion = MapFacade.landLayer.localToGlobal(postion);
            if (replace)
            {
                postion.x = toPostion.x;
                postion.y = toPostion.y;
        ...
        public static function getMapStruct(mapId : uint = 0) : MapStruct
        {
            ...
            if (mapId <= 0)
            {
                ...
                if (mapStruct == null) mapStruct = MapFacade.mapConfigData.getMapStruct(UserData.instance.loginMapId);
            }
            else
            {
                mapStruct = MapFacade.mapConfigData.getMapStruct(mapId);
            }
            return mapStruct;
        }
